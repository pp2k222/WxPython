...
import re
class Element3View(ElementView):

    def __init__(self, parent=None,data_representation = None,show_remove_button=True,default_offer=Element3(1,'','','','','',0,0,8),*args, **kw):
        super(Element3View, self).__init__(parent=parent,show_remove_button=show_remove_button ,*args, **kw)
    ...
    def InitUI(self,data):
        ...
        self.data_sizer.AddMany([
            (wx.StaticText(self,label="###"),1, wx.FIXED_MINSIZE),(wx.StaticText(self,label="Element3"),1, wx.FIXED_MINSIZE),
    ...
    def GetData(self):
        ...
        for d in self.dodatki:
            if d:
                if d.cenaZL2.GetValue() == '' or re.sub('[^0-9]+', '', d.cenaZL2.GetValue()) == '':
                    cenaZL2 = 0
                else:
                    cenaZL2 = int(re.sub('[^0-9]+', '', d.cenaZL2.GetValue()))
                db_dodatki.append(Dodatki_Element3(
        ...
        if self.cenaZL3.GetValue() == '' or re.sub('[^0-9]+', '', self.cenaZL3.GetValue()) == '':
            ...
            cenaZL3 = int(re.sub('[^0-9]+', '', self.cenaZL3.GetValue()))
        element = Element3(self.value1.GetValue(), self.value2.GetValue(), self.value3.GetValue(), self.value4.GetValue(), self.value5.GetValue(), self.value6.GetValue(), cenaZL1, cenaZL2, cenaZL3, dodatki=db_dodatki, id=self._id)
        return element
