...
import re
class Element4View(ElementView):

    def __init__(self, parent=None,data_representation = None,show_remove_button=True,default_offer = Element4(1,'','','','','','','','','','',0,0,8),*args, **kw):
        super(Element4View, self).__init__(parent=parent ,show_remove_button=show_remove_button,*args, **kw)
        self.data = data_representation
        if self.data == None:
            self._id = None
            self.data = default_offer
            if default_offer.dodatki:
                self.data.dodatki = default_offer.dodatki
            else:
                self.data.dodatki = []
        else:
    ...
    def InitUI(self,data):
        ...
        self.value2 =        wx.TextCtrl(parent=self,value=data.value2,style = wx.TE_PROCESS_ENTER)
        self.value3 = wx.TextCtrl(parent=self,value=data.value4)
        self.value4 = wx.TextCtrl(parent=self,value=data.value3)
        ...
        self.value10 = wx.TextCtrl(parent=self,value=data.value10)
        ...
        self.inputElements.append(self.value10)
        ...
        self.data_sizer.AddMany([
            (wx.StaticText(self,label="###"),1, wx.FIXED_MINSIZE),(wx.StaticText(self,label="Element4"),1, wx.FIXED_MINSIZE),
            ...
            (wx.StaticText(self,label="Wartość 10"),1, wx.FIXED_MINSIZE),(self.value10,1, wx.EXPAND),
    ...
    def GetData(self):
        ...
        for d in self.dodatki:
            if d:
                ...
                db_dodatki.append(Dodatki_Element4(
        ...
        Element4_to_db = Element4(self.value1.GetValue(), self.value2.GetValue(), self.value4.GetValue(), self.value3.GetValue(), self.value5.GetValue(), self.value6.GetValue(),
                          self.value7.GetValue(), self.value8.GetValue(), self.value9.GetValue(), self.value10.GetValue(), self.value9.GetValue(), cenaZL1, cenaZL2, cenaZL3, dodatki=db_dodatki, id=self._id)
        return Element4_to_db
